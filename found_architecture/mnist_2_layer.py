...
C[K:3-F:89-L2:0.0001]
C[K:3-F:90-L2:0.0001]
C[K:3-F:93-L2:0.0005]
P[K:2-S:2]
...
def model():
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected], activation_fn=tf.nn.crelu, normalizer_fn=slim.batch_norm, normalizer_params={'is_training': is_training, 'decay': 0.95}):
        ...
        h = slim.conv2d(inputs=h, num_outputs=89, kernel_size=3, weights_regularizer=slim.l2_regularizer(0.0001))
        h = slim.conv2d(inputs=h, num_outputs=90, kernel_size=3, weights_regularizer=slim.l2_regularizer(0.0001))
        h = slim.conv2d(inputs=h, num_outputs=93, kernel_size=3, weights_regularizer=slim.l2_regularizer(0.0005))
        h = slim.max_pool2d(h, kernel_size=2, stride=2)
